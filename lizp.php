...
class Lizp {
    ...
    public function Compile($sexp) {
        ...
        if ($sexp instanceof Symbol) {
            $this->Emit("\$r = @\$ENVIRONMENT['" . addslashes($sexp->name) . "'];");
            return;
        } elseif (!is_array($sexp)) {
            $this->Emit('$r = ' . Expression::Compile($sexp) . ';');
            return;
        }
        ...
        if ($lambda instanceof Symbol) {
            ...
            $specialForm = "lizp_special_form_{$phpName}";
            if (function_exists($specialForm)) {
                $this->Emit("\$r = $specialForm(\$env, " . Expression::Compile($args) . ");");
                return;
            }
        }
...
class Expression {
    ...
    public static function Compile($expr) {
        if ($expr === NULL || $expr === FALSE ||
            (is_array($expr) && empty($expr))) {
            return 'NULL';
        }
